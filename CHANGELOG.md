...
- **Vanilla CSS Decision Rationale** (production book §5.2) — documented why Vanilla CSS was chosen over Tailwind/Sass: the 4-layer cascade (`Base Light → Dark → Flavor → Flavor+Dark`) maps directly to native CSS custom properties with data-attribute selectors; Tailwind would add a build dependency while still requiring the same cascade; Astro supports native CSS nesting already
- **Theme & Flavor Manager** (production book §5.6) — complete specification for the unified theme/flavor management system: Theme Toggle + FlavorSelector components, anti-flash `<head>` script, `localStorage` persistence, `site-theme-change` and `site-flavor-change` events, `prefers-color-scheme` fallback, quality checklist for all 6 combinations
- **Flavor Preview Page** (production book §5.7) — new `/about/design-preview/` page specification for visual QA of all 6 flavor/theme combinations: sticky controls bar, color palette with auto WCAG contrast, typography samples, full component showcase, page pattern samples, and Issue Reporter (copies formatted markdown to clipboard for Claude Code or designer). Added to navigation menu, site-plan page inventory, and component file structure (`DesignIssueReporter.svelte`)
